...
.notice-box {
  ...
  font-size: 14px;
  margin: 15px 0;
  color: hsl(0, 0%, 66%); /* mas dark red text */
}
...
.footer {
  font-size: 11px;   /* pwede mo pa liitan to 10px kung gusto mo */
  margin-top: 18px;
  opacity: 0.6;
}
...
<div class="container">
  ...
  <div class="payment-group" id="paymentOptions">
    <button class="gcash-btn" onclick="openQR('gcash')">GCash</button>
  </div>

  <div class="contact-btn" onclick="openContact()">💬 Contact Support</div>

  <div class="footer">© 2026 Skytech Internet Installation Services - Kiangan Ifugao | Secure Access Portal</div>
</div>
...
<div id="qrModal" class="modal" onclick="closeAll()">
  <div class="modal-content" onclick="event.stopPropagation()">
    ...
    <img id="qrImage">
    <div> JO••H N.</div>
    <div class="number-box" id="payNumber">0926-937-1159</div>
    ...
    <div class="payment-note">
      ...
      <br><br>Facebook: Skytech Internet Installation Services - Kiangan Ifugao
      <br>Phone: 0926-93791159
    </div>
  </div>
</div>
...
<div id="contactModal" class="modal" onclick="closeAll()">
  <div class="modal-content" onclick="event.stopPropagation()">
    ...
    <a href="tel:+639269371159" class="option-btn call-btn">
      📞 Call Now
    </a>
...
function openQR(type) {
  ...
  if (type === "gcash") {
    qrImage.src = "gcash.png";
    qrTitle.innerText = "Scan to Pay (GCash)";
  }

  if (type === "maya") {
    qrImage.src = "maya.png";
    qrTitle.innerText = "Scan to Pay (Maya)";
  }
...
function copyQRNumber(button) {
  const number = document.getElementById("payNumber").innerText.replace(/-/g, '');
